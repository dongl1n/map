...
import {Control} from './lib/control/Control.js';
import * as DomUtil from './lib/dom/DomUtil.js';
 ...
 */
	...
	export const Scale = Control.extend({
		...
		onAdd(map) {
			const className = 'leaflet-control-scale custom-scale',
				container = DomUtil.create('div', className),
				options = this.options;
			this.options.mInDeg = this.options.kmInDeg * 1000;
		...
		_update() {
			...
			this._updateScales(maxMeters * this.options.mInDeg);
		},
		...
		_updateScale(scale, text, ratio) {
			...
			const mapContainer = document.querySelector(".leaflet-overlay-pane");
			const img = document.querySelector(".leaflet-image-layer");
			const styles = window.getComputedStyle(img);
			const clone = scale.cloneNode(true);
			clone.className += " scale-image";
			if(document.querySelector('.scale-image')){
				document.querySelector('.scale-image').remove();
			}
			clone.style.transform = styles.transform;
			clone.innerHTML += ` = ${clone.style.width}`
			mapContainer.append(clone)
		},
